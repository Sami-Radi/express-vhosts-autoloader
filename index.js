...
var Autoloader = function () {
   ...
   * directory name. e.g : www.virtuoworks.com, module name defaults to app.js,
  ...
  this.loadVhost = function loadVhost(settings) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (this.expressServer && typeof this.expressServer === "function"
        ...
        settings.directory = settings.directory || path.normalize(process.cwd() + path.sep);

        if (typeof settings.directory !== "string") {
          let message = `[${tag}] "settings.directory" must be a string.`;
          return reject(new TypeError(message));
        }

        let moduleName = path.normalize(settings.directory + path.sep + settings.domainName + path.sep + settings.mainFile);
  ...
  /**
   * Loads a virtual host middleware according to directory name (e.g :
   * directory www.virtuoworks.com will be bound to www.virtuoworks.com virtual
   * host).
   *
   * @param {string} fileOrDirectory A file or directory name.
   * @param {object} settings (optional) A settings object.
   * @param {string} settings.directory (optional) A string representing the directory
   * where the virtual hosts directories can be found. Defaults to server directory.
   * @param {string} settings.ignoreList (optional) An array of directories to ignore.
   * @return {object} A Promise object.
   */
  this.loadAsMiddleWare = function loadAsMiddleWare (fileOrDirectory, settings) {
    settings = settings || {};
    return new Promise((resolve, reject) => {

      if (typeof fileOrDirectory !== "string") {
        let message = `[${tag}] "fileOrDirectory" is expected to be a string.`;
        return reject(new TypeError(message));
      }
      ...
      settings.directory = settings.directory  || settings.folder || path.normalize(process.cwd() + path.sep);
      ...
      settings.ignoreList = settings.ignoreList || defaultIgnoreList;

      if (!Array.isArray(settings.ignoreList)) {
        let message = `[${tag}] "settings.ignoreList" must be an Array.`;
        logger.error(message);
        return reject(new TypeError(message));
      }

      fs.stat(path.normalize(settings.directory + path.sep + fileOrDirectory), (error, stats) => {
        if (error) {
          let message = `[${tag}] Cannot read : ${fileOrDirectory}.`;
          logger.warn(message);
          return reject({
            message,
            error
          ...
          if (stats.isDirectory() && -1 === settings.ignoreList.indexOf(fileOrDirectory)) {
            let moduleFile = path.normalize(settings.directory + path.sep + fileOrDirectory + path.sep + "app.js");
            fs.access(moduleFile, fs.R_OK, (error) => {
              if (error) {
            ...
            let message = `[${tag}] ${fileOrDirectory} is not a directory.`;
            reject({
              message
            });
          }
  ...
  /**
   * Auto detects and loads virtual hosts according to directory name (e.g :
   * directory www.virtuoworks.com will be bound to www.virtuoworks.com virtual
   * host).
   *
  ...
  this.run = function run (expressServer, settings) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (expressServer && typeof expressServer === "function" && expressServer.use) {
        ...
        fs.readdir(settings.directory, (error, files) => {
          if (error) {
            let message = `[${tag}] Cannot read the current working directory : ${settings.directory}.`;
            logger.error(message);
            return reject(new Error(message));
          } else {
            let scan = [];
            files.forEach((fileOrDirectory) => {
              scan.push(new Promise((resolve, reject) => {
                this.loadAsMiddleWare(fileOrDirectory, settings).then((success) => {
                  resolve(success);
                }, (error) => {
                  // Ignore all errors.
                  resolve(error);
